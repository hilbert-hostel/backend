...
import { DoorLockCodeDecodeInput, DoorLockCodeEncodeInput } from './door.interface';

export interface IDoorLockCodeService {
	getDoorLockInput(userID: string, roomID: string): Promise<DoorLockCodeEncodeInput>
...
export class DoorLockCodeService implements IDoorLockCodeService {
	...
	async getDoorLockInput(userID: string, roomID: string) {
		...
		if (!user)
			throw new BadRequestError(
				'Request failed. User ID is invalid.'
			)
		const secret = this.generateTOTP(userID+roomID.padStart(4,'0')+user.national_id, 300).toString(); // valid for 300 windows
		return {
			userID: userID,
	...
	encode(input: DoorLockCodeEncodeInput) {
		return input.userID+'|'+input.roomID+'|'+input.nationalID+'|'+input.secret;
	}

	verify(input: DoorLockCodeDecodeInput) {
		const [userID, roomID, nationalID, totp] = input.encoded.split('|');
